...
def Process_HubShroud_IGES():
    ...
    iges_case = pyiges.read(str(script_dir / 'case.igs'))
    iges_hub = pyiges.read(str(script_dir / 'hub.igs'))
    ...
    np.savetxt(str(script_dir / 'shroud.csv'),case_pts,fmt="%f",delimiter=',',header='x,r,theta')
    np.savetxt(str(script_dir / 'hub.csv'),hub_pts,fmt="%f",delimiter=',',header='x,r,theta')
...
def Process_StatorRotor_IGES():
    ...
    iges_rotor1 = pyiges.read(str(script_dir / 'hpt_stator1.igs'))
    iges_stator1 = pyiges.read(str(script_dir / 'hpt_rotor1.igs'))

    iges_rotor2 = pyiges.read(str(script_dir / 'hpt_stator2.igs'))
    iges_stator2 = pyiges.read(str(script_dir / 'hpt_rotor2.igs'))
    curve_delta=0.001
    # Stage 1
    ...
    for i in range(2,7):
        ...
        np.savetxt(str(csv_dir / f'stator1_{indx}.csv'),points,fmt="%f",delimiter=',',header='x,rtheta,r')
        indx+=1
    ...
    for i in range(2,7):
        ...
        np.savetxt(str(csv_dir / f'rotor1_{indx}.csv'),points,fmt="%f",delimiter=',',header='x,rtheta,r')
        indx+=1
    ...
    for i in range(2,6):
        ...
        np.savetxt(str(csv_dir / f'stator2_{indx}.csv'),points,fmt="%f",delimiter=',',header='x,rtheta,r')
        indx+=1
    ...
    for i in range(2,7):
        ...
        np.savetxt(str(csv_dir / f'rotor2_{indx}.csv'),points,fmt="%f",delimiter=',',header='x,rtheta,r')
        indx+=1

    pickle.dump([stator_pts1,rotor_pts1,stator_pts2,rotor_pts2],open(str(script_dir / 'stator_rotor.pkl'),'wb'))
...
if __name__ == "__main__":
    ...
    blades = pickle.load(open(str(script_dir / 'stator_rotor.pkl'),'rb'))
    processed_data = []
    npts = 400
    ...
    for i in range(len(processed_data)):
        ...
        agf = AGF_Setup(template_file=str(script_dir / 'template.agf'),name=f"EEE-HPT-{i}")
        ...
        agf.add_blade(ss,ps)
        agf.build(output_filename=str(script_dir / f'blade{i}.agf'))

    # Run Wand
    result = subprocess.run(str(script_dir / 'runwand.sh'), shell=True, capture_output=True, text=True)
    print(result.stdout)
    # # Check for negative cells

    # Print the output
    with open(str(script_dir / "wand.stdout"), "w") as file:
        file.write(result.stdout)
    # Print any errors
    if "NO NEGATIVE VOLUME in result.stdout":
        result = subprocess.run(str(script_dir / 'runleo.sh'), shell=True, capture_output=True, text=True)
        print(result.stdout)
        with open(str(script_dir / "leo.stdout"), "w") as file:
            file.write(result.stdout)
