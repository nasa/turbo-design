...
def main() -> None:
    ...
    circumference = 2*np.pi*rmean 
    number_of_blades = circumference/cax/solidity 
    ...
    rotor.loss_function = FixedPressureLoss(phi_rotor)

    stator = BladeRow(hub_location=2*cax/max(xhub_arr), row_type=RowType.Stator)
    stator.beta2_metal = [alpha1]
    stator.loss_function = FixedPressureLoss(phi_stator)
    stator.P0_ratio = 1.3
